...
channel_index = ['Fp1', 'Fp2', 'AF3', 'AF4', 'F7', 'F8', 'F3', 'Fz', 'F4', 'FC5', 'FC6', 'T7', 'T8', 'C3',
...
for index in range(len(set_file_name)):  # 处理被试者某个子试验的数据
    ...
    for i in range(len(mark) - 1):  # 依次处理这次子试验下的所有点击动作
        ...
        sliding_window = duration // WINDOWS + 1  # 以300ms秒为一个窗口截取数据计算psd，不足的当作一段

        for seg in range(sliding_window):
            section_dir = name_dir + '\\{0:0>2}'.format(seg)
        ...
        for seg in range(sliding_window):
            ...
            if start != end and (end - start) > 120:
                df = pd.DataFrame(raw_eeg[:, -end:-(start + 1)], index=channel_index)
                df.to_csv(Section_dir[seg] + '\\{0:0>4}.csv'.format(nums))
